...
export class GeoJsonUtils {
    ...
    parseLinearRingOrLineString = (_, opts, ctx = {}) => { // or a LineStringSegment
        ...
        if (points.length === 0) throw new Error(_.nodeName + ' must have > 0 points')
        return points
    }
    ...
    parseRing = (_, opts, ctx = {}) => {
        ...
        if (points.length < 4) throw new Error(_.nodeName + ' must have >= 4 points');
        return points
    }
    ...
    parseExteriorOrInterior = (_, opts, ctx = {}) => {
        ...
        if (ring) return this.parseRing(ring, opts, ctx)
    ...
    parsePolygonOrRectangle = (_, opts, ctx = {}) => { // or PolygonPatch
        ...
        if (!exterior) throw new Error('invalid ' + _.nodeName + ' element')
    ...
    parseSurface = (_, opts, ctx = {}) => {
        ...
        if (polygons.length === 0) throw new Error(_.nodeName + ' must have > 0 polygons')
        return polygons
    }
