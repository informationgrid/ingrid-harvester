...
export class PostgresAggregator implements AbstractPostgresAggregator<ZdmIndexDocument> {
    ...
    public async processBucket(bucket: Bucket<ZdmIndexDocument>): Promise<EsOperation[]> {
        ...
        if (deleteDocument) {
            return [{ operation: 'delete', _index: document['catalog'].identifier, _id: document.t01_object.obj_id }];
        }
        ...
        box.push({ operation: 'index', _index: document['catalog'].identifier, _id: document.t01_object.obj_id, document });
        return box;
    }
