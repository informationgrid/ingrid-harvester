...
export class WfsImporter extends Importer<WfsSettings> {
    ...
    async extractFeatureType(featureTypeName: string, featureTypeNode: Node, featureTypeDescriptionNode: Node, numFeatures: number) {
        ...
        if (this.settings.harvestTypes) {
            this.observer.next(this.summary.msgRunning(++this.numIndexDocs, this.numItems, 'FeatureTypes werden heruntergeladen'));
        }
    }
